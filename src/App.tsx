  ...
  useEffect(() => {
    const loadPrayerTimes = async () => {
      try {
        ...
        if (location) {
          const schedule = await fetchPrayerTimes(location.latitude, location.longitude);
    ...
    // Set up interval to refresh prayer times every minute
    const intervalId = setInterval(loadPrayerTimes, 60000);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-white transition-colors duration-200">
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div className="mb-6 flex justify-end">
          ...
          >
            <MapPin size={16} className="mr-1 text-emerald-600 dark:text-emerald-400" />
            {showLocationSelector ? 'Hide Locations' : 'Change Location'}
          </button>
        </div>
